...
class ServiceProjectController extends Controller
{
    public function index(Request $request){
        return view('backend.service.serviceproject.index');
    }
    /**
     * 列表数据
    ...
    public function getdata(Request $request){
        ...
        $query = ServiceProject::where('type',1)->select();
        $total = $query->count();
    ...
    public function create(Request $request){
        if($request->isMethod('post')){
            ...
            if($opto->save()){
                return redirect()->route('admin.service_project.index')->withFlashSuccess('添加成功');
            }else{
...
//        $dat = $this->gettree($data,0);
    ...
    public function info($id){
        ...
        return view('backend.service.serviceproject.show',['project'=>$data]);
    }
    /**
     * 修改数据
     * @param $id
    ...
    public function edit(Request $request){
        ...
        if($data->save()){
            return redirect()->route('admin.service_project.index')->withFlashSuccess('修改成功');
        }else{
