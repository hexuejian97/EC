...
class OptometryController extends Controller
{
    ...
    public function index(){
        return view('backend.service.optometry.index');
    }
    ...
    public function getdata(Request $request){
        ...
        $query = Optometry::where('type',1)->select();
        $total = $query->count();
    ...
    public function create(Request $request){
        if($request->isMethod('post')){
            ...
            if($opto->save()){
                return redirect()->route('admin.optometry.index')->withFlashSuccess('添加成功');
            }else{
                return back()->withErrors('添加医师失败，请重新添加');
            }
        }
        return view('backend.service.optometry.create');
    }
    ...
    public function info($id){
        ...
        return view('backend.service.optometry.show',['optometry'=>$data]);
    }
    ...
    public function update($id){
        $data = Optometry::find($id);
        return view('backend.service.optometry.edit',['optometry'=>$data]);
    }
    ...
    public function edit(Request $request){
        ...
        if($data->save()){
            return redirect()->route('admin.optometry.index')->withFlashSuccess('修改成功');
        }else{
